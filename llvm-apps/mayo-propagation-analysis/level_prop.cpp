...
struct Env {
  map<Value *, Level> reg;
  map<Value *, Level> mem;
  map<pair<Value *, unsigned>, Level> fieldMem;
...
private:
  ...
  Env analyzeFunc(Function *F, Env env) {
    ...
    for (auto &BB : *F) {
      for (auto &I : BB) {
        if (auto *A = dyn_cast<AllocaInst>(&I)) {
          ...
          if (name == "sk" || name == "esk") {
            env.mem[A] = Level::Public; // default for unrecognized fields
            env.reg[A] = Level::Public;
            env.fieldMem[{A, 0}] = Level::Public;
            env.fieldMem[{A, 1}] = Level::Secret;
            continue;
          }
          if (name == "O" || name == "L" || name == "Ox" || name == "seed_sk" ||
      ...
      for (auto &BB : *F) {
        for (auto &I : BB) {
          ...
          // gep
            ...
            if (G->getNumIndices() >= 2) {
              ...
              if (auto *CI = dyn_cast<ConstantInt>(idxIt->get())) {
                ...
                auto key = make_pair(base, fieldIdx);
                Level fieldLvl =
                    env.fieldMem.count(key) ? env.fieldMem[key] : env.mem[base];
                env.reg[&I] = fieldLvl;
                env.mem[&I] = fieldLvl;
                continue;
              }
          ...
          // load
            ...
            if (auto *G = dyn_cast<GetElementPtrInst>(ptr)) {
              if (G->getNumIndices() >= 2) {
                ...
                if (auto *CI = dyn_cast<ConstantInt>(idxIt->get())) {
                  unsigned fieldIdx = CI->getZExtValue();
                  Value *structBase = getBase(G->getPointerOperand());
                  auto key = make_pair(structBase, fieldIdx);
                  Level fieldLvl = env.fieldMem.count(key)
                                       ? env.fieldMem[key]
                                       : env.mem[structBase];
                  env.reg[&I] = fieldLvl;
          ...
          // store
            ...
            Value *dstPtr = S->getPointerOperand();
            Value *dst = getBase(dstPtr);
            bool handled = false;

            if (auto *G = dyn_cast<GetElementPtrInst>(dstPtr)) {
              if (G->getNumIndices() >= 2) {
                auto idxIt = G->idx_begin();
                ++idxIt;
                if (auto *CI = dyn_cast<ConstantInt>(idxIt->get())) {
                  ...
                  Value *structBase = getBase(G->getPointerOperand());
                  auto key = make_pair(structBase, fieldIdx);
                  env.fieldMem[key] = join(env.fieldMem[key], env.reg[src]);
                  handled = true;
                }
              }
            }
    ...
    // outs() << "\n--- Function: " << F->getName() << " ---\n";
    // if (callStack.size() >= 2) {
    //   outs() << "Called from: " << callStack[callStack.size() -
    //   1]->getName()
    //          << "\n";
    // } else {
